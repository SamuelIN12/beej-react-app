...
// getModalStyle is not a pure function, we roll the style only on the
...
//access the storage in firebase, get a references to the folder
//images/ and store image there
...
useEffect(() => {
const unsubscribe = auth.onAuthStateChanged((authUser) => { //listen
//anytime an authentication change happens
if (authUser) {
//user has logged in...
console.log(authUser); //check the console if someone is there or not
setUser(authUser); //Cookie tracking to keep you logged in. Captures
//the user in our state.
...
return () => {
//perform some cleanup actions before restarting the useEffect. This
...
onClose={() => setOpen(false)} //onClose method. closes the model when
anywhere else on the screen is clicked
>
...
onClose={() => setOpenSignIn(false)} //onClose method. closes the
model when anywhere else on the screen is clicked
>
<div style={modalStyle} className={classes.paper}>
<form className= "app__signup">
<center>
...
onClose={() => setOpenP(false)} //onClose method. closes the model
when anywhere else on the screen is clicked
>
<div style={modalStyle} className={classes.paper}>
...
<input type="text" placeholder='Enter a caption...' onChange={event =>
setCaption(event.target.value)} value={caption}/>
...
caption={post.caption} imageUrl={post.imageUrl} keyword={post.keyword}
></Post>
))
}
</div>
